    ...
    <div>
        ...
        <div>
            <el-table :data="tableData" style="width: 100% " height="220">
                <el-table-column prop="name" label="成就与亮点" width="120">
                </el-table-column>
                <el-table-column prop="score" label="分数" width="60px">
                </el-table-column>
                <el-table-column prop="reason" label="得分原因" width="500">
                </el-table-column>
            </el-table>
        </div>
    </div>
...
<script>
    ...
    export default {
        name: 'ChartSkillsAndExperiences',
        mounted() {
            this.renderChart();
        },
        data() {
        ...
        methods: {
            renderChart() {
                ...
                axios.post(this.$store.state.baseURL + '/Applicants/graph/SkillsAndExperiences', { id: this.RId })
                    .then(response => {
                        ...
                        //将这个动态赋值
                        for (var i = 0; i < 3; i++) {
                            if (data[i].name === "问题解决能力") { scoreData.push(data[i].score); }
                        ...
                        for (var i = 0; i < 3; i++) {
                            ...
                            if (data[i].name === "团队协作能力") { scoreData.push(data[i].score); continue; }
                            //if(data[i].name==="社交能力") {scoreData.push(data[i].score);continue;}
                        ...
                        for (var i = 0; i < 3; i++) {
                            //if(data[i].name==="自我驱动性") {scoreData.push(data[i].score);}
                            //if(data[i].name==="适应能力") {scoreData.push(data[i].score);continue;}
                            if (data[i].name === "创新思维") { scoreData.push(data[i].score); continue; }
