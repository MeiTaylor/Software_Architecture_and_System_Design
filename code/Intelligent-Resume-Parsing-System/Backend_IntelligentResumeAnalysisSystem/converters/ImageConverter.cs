    ...
    public class ImageConverter : IFileConverter
    {
        public void ConvertToText(string filePath)
        {
            try
            {
                ...
                string pythonScriptPath = Path.Combine(binDirectory, @"..\..\..\python_scripts\img_to_txt.py");
                ...
                using (Process process = Process.Start(start))
                {
                    ...
                    using (StreamReader reader = process.StandardOutput)
                    {
                        ...
                        {
                            Console.WriteLine($"图像文件转txt文件出错: {stderr}");
                        }
                        else
                        {
                            Console.WriteLine("图像文件转txt文件成功");
                        }
                    }
                }
            ...
            {
                // 处理异常
                Console.WriteLine($"在转换图片Image文件时发生错误: {ex.Message}");
            }
        }
    }
}
