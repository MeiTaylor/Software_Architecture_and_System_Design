...
{
    public class BasicInfoParser : IResumeParser
    {
        public void ParseAndSaveAsJson(string filePath)
        {
            try
            {
                ...
                string promptFilename = "Basic_Infos_Guide"; // 提示文件名
                ...
                using (Process process = Process.Start(start))
                {
                    ...
                    using (StreamReader reader = process.StandardOutput)
                    {
                        ...
                        {
                            Console.WriteLine($"基本信息分析出错: {stderr}");
                        }
                        else
                        {
                            Console.WriteLine("基本信息分析成功");
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine($"在进行基本信息分析时发生错误: {ex.Message}");
            }
